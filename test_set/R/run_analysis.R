...
# should we use h2oboost or xgboost?
h2oboost <- TRUE
if (h2oboost) {
    library("h2o")
    h2o::h2o.init()
...
all_mab_files_2 <- list.files(here("test_set", "docker_output", 
                                   "10-1074_3bnc117_2"))
files_to_use <- switch((h2oboost) + 1, grepl("Jan", all_mab_files_2), 
                       grepl("09Apr", all_mab_files_2))
training_data_2 <- readr::read_csv(
    here("test_set", "docker_output", "10-1074_3bnc117_2", 
         all_mab_files_2[grepl("slapnap", all_mab_files_2) & files_to_use])
)
learner_files_2 <- all_mab_files_2[grepl("learner", all_mab_files_2) & 
                                   !grepl("cv", all_mab_files_2) & files_to_use]
...
cv_learner_files_2 <- all_mab_files_2[grepl("cvlearner", all_mab_files_2) & files_to_use]
...
# if used h2oboost, load models
if (h2oboost) {
    h2o_models <- all_mab_files_2[grepl("GBM_model", all_mab_files_2)]
    for (mod in h2o_models) {
        h2o::h2o.loadModel(here("test_set", "docker_output", 
                                "10-1074_3bnc117_2", mod))
    }
}
...
# read in cross-validated fits; first, copy files to names that internal SLAPNAP expects
file.copy(from = here("test_set", "docker_output", "10-1074_3bnc117_2",
                      cv_learner_files_2[grepl("ic50", learner_files_2)]),
          to = here("test_set", "docker_output", "10-1074_3bnc117_2",
                    "cvfit_log10.pc.ic50.rds"))
file.copy(from = here("test_set", "docker_output", "10-1074_3bnc117_2",
                      cv_learner_files_2[grepl("estsens", learner_files_2)]),
          to = here("test_set", "docker_output", "10-1074_3bnc117_2",
                    "cvfit_dichotomous.1.rds"))
file.copy(from = here("test_set", "docker_output", "10-1074_3bnc117_2",
                      cv_learner_files_2[grepl("multsens", learner_files_2)]),
          to = here("test_set", "docker_output", "10-1074_3bnc117_2",
                    "cvfit_dichotomous.2.rds"))
...
#         opts = list(learners = c("rf", "lasso", "h2oboost")))

# ------------------------------------------------------------------------------
# estimate test-set performance (using AUC, accuracy for binary outcomes; R-squared for IC50)
# ------------------------------------------------------------------------------
preds_ic50 <- predict(learner_ic50, newdata = cc_covars, onlySL = TRUE)$pred
preds_estsens_2 <- predict(learner_estsens_2, newdata = cc_covars, 
                           onlySL = TRUE)$pred
preds_multsens_2 <- predict(learner_multsens_2, newdata = cc_covars, 
                            onlySL = TRUE)$pred
# preds_estsens_1 <- predict(learner_estsens_1, newdata = cc_covars, 
#                            onlySL = TRUE)$pred
# preds_multsens_1 <- predict(learner_multsens_1, newdata = cc_covars, 
#                             onlySL = TRUE)$pred

# close the h2o API connection
if (h2oboost) {
...
summ_estsens_2 <- subset.summary.myCV.SuperLearner(object = fit_list_out$out$sens1,
                                 method = "method.AUC", 
                                 opts = list(learners = c("rf", "h2oboost", "lasso")),
...
ci_ic50 <- vimp::vimp_ci(point_est_ic50,
                         se = sqrt(mean(perf_ic50$eif ^ 2)) / length(perf_ic50$eif))
ci_estsens_2 <- vimp::vimp_ci(point_est_estsens_2,
                         se = sqrt(mean(perf_estsens_2$eif ^ 2) / length(perf_estsens_2$eif)),
                         scale = "logit")
ci_multsens_2 <- vimp::vimp_ci(point_est_multsens_2,
                         se = sqrt(mean(perf_multsens_2$eif ^ 2) / length(perf_multsens_2$eif)),
                         scale = "logit")
